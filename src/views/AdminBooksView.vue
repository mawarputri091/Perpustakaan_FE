...
const handleReject = async (id) => {
  ...
  if (result.isConfirmed) {
    ...
    if (success) {
      ...
      Swal.fire({
        title: 'Gagal!',
        text: 'Gagal menolak peminjaman. Coba lagi.',
        icon: 'error',
        confirmButtonColor: '#ef4444'
      })
    }
...
const handleReturnBook = async (loanId) => {
  ...
  if (success) {
    ...
    Swal.fire({
      title: 'Gagal!',
      text: 'Gagal memproses pengembalian buku. Coba lagi.',
      icon: 'error',
      confirmButtonColor: '#ef4444'
    })
  }
...
const selectedPdfFile = ref(null) // 🌟 STATE BARU: Menampung file PDF

const handleFileChange = (event) => {
  selectedFile.value = event.target.files[0] || null
}

// 🌟 FUNGSI BARU: Menangani perubahan input file PDF
...
const openAddModal = () => {
  ...
  selectedPdfFile.value = null // Reset PDF
...
const saveBook = async () => {
  ...
  const payload = { 
    ...
    type: newBook.type,
    stock: newBook.type === 'digital' ? 0 : newBook.stock, // E-book set stok ke 0 otomatis
    harga: newBook.harga,
    description: newBook.description,
    cover: newBook.cover,
    file: selectedFile.value, // File Cover Gambar
    file_pdf: selectedPdfFile.value, // 🌟 DATA BARU: Dikirim ke store backend
    pdfUrl: DEFAULT_PDF, 
    rating: 0, 
    reviews: [] 
  }
          ...
          <form @submit.prevent="saveBook" class="space-y-4">
            ...
            <div class="flex gap-4">
              ...
              <div class="w-1/2" v-if="newBook.type === 'digital'">
                ...
                <p v-if="isEditMode" class="text-[10px] text-slate-500 mt-1">Abaikan jika tidak ingin mengubah file PDF.</p>
              </div>
            </div>
