  ...
  // ✅ FIX: cek hasil returnBook() — sebelumnya selalu menampilkan alert sukses
  // walau request ke BE gagal.
  ...
  const processOfflineLoan = async () => {
    ...
    // ✅ FIX: tampilkan pesan error ASLI dari backend (result.message),
    // bukan selalu "Stok mungkin habis" untuk semua jenis kegagalan.
          ...
          <table class="w-full text-left text-sm border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="loan in activeLoans" :key="loan.id" class="hover:bg-slate-50 transition">
                <td class="p-3 font-medium text-slate-800">
                  <!-- ✅ FIX: untuk transaksi offline (siswa_id null), tampilkan nama_peminjam_offline -->
